    ...
    <article class="report-card" id="report-${report.id}">
      <div class="report-header">
        ...
        <div class="report-meta">
          ...
          ${canModerate || canEdit || canDelete ? `
            <button class="report-menu-btn" onclick="toggleReportMenu('${report.id}')">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="report-dropdown" id="menu-${report.id}">
              ${canEdit ? `<button class="dropdown-item" onclick="editReport('${report.id}')">Editar</button>` : ''}
              ${canDelete ? `<button class="dropdown-item" onclick="deleteReport('${report.id}')">Eliminar</button>` : ''}
              ${canModerate ? `
                <button class="dropdown-item" onclick="approveReport('${report.id}')">Aprobar</button>
                <button class="dropdown-item" onclick="markAsFalse('${report.id}')">Marcar como falso</button>
                <button class="dropdown-item danger" onclick="deleteReportAdmin('${report.id}')">Eliminar (Admin)</button>
              ` : ''}
            </div>
          ` : ''}
        </div>
      ...
      <div class="report-footer">
        <div class="report-stats">
          ...
          <button class="stat-btn ${canVoteBtn ? '' : 'disabled'}" onclick="likeReport('${report.id}')" ${canVoteBtn ? '' : 'disabled'}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
async function markAsFalse(reportId) {
  if (!isAdmin()) {
    showToast('No tienes permisos para esta acción', 'error');
    return;
  }
